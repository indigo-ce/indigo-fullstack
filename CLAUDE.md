...
- **Optional dev server hosts**: `ASTRO_DEV_ALLOWED_HOSTS` is a comma-separated list read from `process.env` in `astro.config.mjs` and passed to `vite.server.allowedHosts` (dev-time only, not a Worker binding)
