...
**The worker-manifest half has drifted open again.** When this item was last revised, `workers/indigo-email-queue-consumer/package.json` carried the root's own range strings. The root has since moved (`wrangler` to `^4.131.0`, `@cloudflare/workers-types` to the v5 line) and that manifest has not. Re-aligning it is **item 35**, not this one — keep the two apart, since 35 edits a manifest nothing installs and cannot change the lockfile, while this item changes what resolves. The runtime half below is all that remains here, and it is not movable from inside this repository today.
