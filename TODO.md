...
1. **37** — repair the new-project checklist and the bootstrap file lists.
...
Re-check the parked pair on each planning pass by reading the pool's newest published release and the `miniflare`/`workerd` it pins: the moment one ships on the line the root `wrangler` already resolves, 27 becomes the first item to pick up and 17 follows it. As of 2026-09-16 the committed `pnpm-lock.yaml` still resolves the pool at 0.22.0, whose entry pins `wrangler@4.124.0` and `miniflare@5.20260815.0-alpha` / `workerd@1.20260815.1` — unchanged by the dependency bump that landed as #83, which moved the root `wrangler` to `^4.131.0` (resolving `4.131.1` with `miniflare@5.20260911.0-alpha` / `workerd@1.20260911.1`) and left the pool's line where it was. Re-derived today: the lockfile carries two `wrangler` keys (`4.124.0`, `4.131.1`) and three each of `miniflare` (`5.20260815.0-alpha`, `5.20260910.0-alpha`, `5.20260911.0-alpha`) and `workerd` (`1.20260815.1`, `1.20260910.1`, `1.20260911.1`). That is the floor, not the check — the registry read is what decides, and it has still not been performed since 2026-09-09, so whoever picks up 27 does that read first and records the version they saw.
